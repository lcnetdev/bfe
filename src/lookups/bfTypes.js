bfe.define('src/lookups/bfTypes', ['require', 'exports', 'src/lookups/lcshared', 'src/bfelogging'], function (require, exports) {
    var lcshared = require('src/lookups/lcshared');
    
    var listitems = [];

    // This var is required because it is used as an identifier.
    exports.scheme = 'http://id.loc.gov/vocabulary/bfTypes';
  
    exports.source = function (query, processSync, processAsync) {
        // console.log(JSON.stringify(formobject.store));

        var scheme = 'http://id.loc.gov/vocabulary/bfTypes';
        ...
        if (listitems.length > 0) {
            ...
            setTimeout(
                function () {
                    var url = 'https://id.loc.gov/ontologies/bibframe.json';
                    $.ajax({
                        url: url,
                        dataType: 'json',
